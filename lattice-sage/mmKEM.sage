...
while flag == 0:
    ...
    d_u = logq
    dec_error = 2*((n+m) * d * nu * sigma_1 * tau + sigma_2 * tau + round(2^logq / 2^(d_u + 1)) )+ 1
    if dec_error < 2^logq / 4 :
        ...
        print("mmKEM: n = ", n)
        print("mmKEM: public parameter |pp| size (KB) = ", round((float(d*logq*n^2)/8/1024),3))
        print("mmKEM: public key |pk| size (KB) = ", round((float(d*logq*n)/8/1024),3))

        d_u = 0
        dec_error_1 = ((n+m) * d * nu * sigma_1 * tau + sigma_2 * tau + nu * d * n * round(2^logq / 2^(d_u + 1)) + round(2^logq / 2^(d_u + 1)) )*2 + 1
        while dec_error_1 >= 2^logq / 4 and d_u <= logq:
            d_u = d_u + 1
            dec_error_1 = ((n+m) * d * nu * sigma_1 * tau + sigma_2 * tau + nu * d * n * round(2^logq / 2^(d_u + 1)) + round(2^logq / 2^(d_u + 1)) )*2 + 1
        print("d_u = ", d_u)
        print("mmKEM: |ct_0| (KB) = ", round((float(d*d_u*(n))/8/1024), 3))
        print("mmKEM: |ct_i| (KB) = ", round((float(d*d_v)/8/1024), 3))
        print("mmKEM: total |ct| (KB) = ", round((float(d*d_v*ell + d*d_u*(n))/8/1024), 3))
        print("mmKEM-comp: compress rate k = ", round(float(ell * 768*8* sec/128)/float(d*d_u*n + d*d_v*ell),3) )
